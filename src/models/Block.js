 ...
 */
  ...
  addTransaction(tx) {
    ...
    //检查输入者的账户余额是否充足
    if (!this.utxoPool.isValidTransaction(tx) && !tx.hasValidSignature()) {
    ...
    // this._setHash()
      ...
      for(let tempUtxo of Object.values(this.utxoPool.utxos)){
        if (tempUtxo.address==tx.inputPublicKey) {
          if (tempUtxo.value>=(tx.amount + tx.fee)) {
      ...
      newUTXO.value = (this.utxoPool.utxos[inputUTXOId].value - tx.amount - tx.fee)
