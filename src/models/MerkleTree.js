...
class MerkleTree {
    constructor(elements) {
        ...
        // 递归计算根哈希值
        this.rootHash = this._buildTree(this.elements);
    }
    ...
    _buildTree(elements) {
        ...
        //只有一个元素
            ...
            const leftHash = this._buildTree(elements.slice(0, mid));
            const rightHash = this._buildTree(elements.slice(mid));  //mid ~ elements.length
...
console.log('Merkle Root:', merkleTree.rootHash); // 输出 Merkle 根节点的哈希值
//添加元素
